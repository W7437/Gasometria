(() => {
  ...
  function analyze() {
    ...
    const summary = [];
    const vals = [];
    if (d.ph !== null) vals.push(`pH ${fmt(d.ph,2)}`);
    if (d.paco2 !== null) vals.push(`PaCO2 ${fmt(d.paco2)} mmHg`);
    if (d.hco3 !== null) vals.push(`HCO3 ${fmt(d.hco3)} mEq/L`);
    if (d.pao2 !== null) vals.push(`PaO2 ${fmt(d.pao2)} mmHg`);
    if (d.lactate !== null) vals.push(`Lactato ${fmt(d.lactate)} mmol/L`);
    if (vals.length) summary.push(`Gasometria: ${vals.join(' | ')}.`);
    summary.push(`${classification.title}.`);
    if (comp) summary.push(comp.map(x => x.text).join(' '));
    if (ag) summary.push(ag.text);
    if (oxy) summary.push(oxy.text + '.');
    if (perf) summary.push(perf.text + '.');
    summaryText.value = summary.join(' ').replace(/\s+/g, ' ').trim();
